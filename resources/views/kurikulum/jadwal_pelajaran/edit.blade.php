{{-- resources/views/kurikulum/jadwal_pelajaran/edit.blade.php --}}
@extends('layouts.app')
    ...
    <div class="page-header">
        <div class="d-flex align-items-center justify-content-between flex-wrap">
            <div>
                ...
                <div class="text-muted">Perbarui data jadwal pelajaran</div>
            </div>
        ...
        <div class="card-body">
            <div class="mb-3">
                <span class="badge-current-info">
                    ...
                    {{ $jadwal->hari }} - {{ $jadwal->jamPelajaran?->kode_jam }} - {{ $jadwal->mataPelajaran?->nama }}
                </span>
            </div>

            <form action="{{ route('kurikulum.jadwal-pelajaran.update', $jadwal->id) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('semester_id') is-invalid @enderror" id="semester_id" name="semester_id">
                            ...
                            @foreach($semesters as $s)
                                <option value="{{ $s->id }}" {{ old('semester_id', $jadwal->semester_id) == $s->id ? 'selected' : '' }}>
                                    {{ $s->label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select class="form-select @error('rombel_id') is-invalid @enderror" id="rombel_id" name="rombel_id">
                            ...
                            @foreach($rombels as $r)
                                <option value="{{ $r->id }}" {{ old('rombel_id', $jadwal->rombel_id) == $r->id ? 'selected' : '' }}>
                                    {{ $r->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select class="form-select @error('jam_pelajaran_id') is-invalid @enderror" id="jam_pelajaran_id" name="jam_pelajaran_id">
                            ...
                            @foreach($jamPelajarans as $jp)
                                <option value="{{ $jp->id }}" {{ old('jam_pelajaran_id', $jadwal->jam_pelajaran_id) == $jp->id ? 'selected' : '' }}>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('ruang_kelas_id') is-invalid @enderror" id="ruang_kelas_id" name="ruang_kelas_id">
                            ...
                            @foreach($ruangKelas as $rk)
                                <option value="{{ $rk->id }}" {{ old('ruang_kelas_id', $jadwal->ruang_kelas_id) == $rk->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('is_active') is-invalid @enderror" id="is_active" name="is_active">
                            ...
                            <option value="0" {{ old('is_active', $jadwal->is_active) == '0' ? 'selected' : '' }}>Tidak Aktif</option>
                        </select>
                        @error('is_active')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('keterangan') is-invalid @enderror"
                              ...
                              name="keterangan"
                              rows="2"
                              placeholder="Masukkan keterangan (opsional)">{{ old('keterangan', $jadwal->keterangan) }}</textarea>
                    @error('keterangan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
