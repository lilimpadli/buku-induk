...
@section('title', 'Edit Mata Pelajaran TU Kepegawaian')
    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('tu_kepegawaian.mata-pelajaran.update', $mataPelajaran->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                                        ...
                                        @foreach($kurikulums as $k)
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" name="kurikulum_ids[]" value="{{ $k->id }}" id="kurikulum_{{ $k->id }}"
                                                       {{ $mataPelajaran->kurikulums->contains($k->id) ? 'checked' : '' }}>
                                        ...
                                        @foreach($jurusans as $j)
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" name="jurusan_ids[]" value="{{ $j->id }}" id="jurusan_{{ $j->id }}"
                                                       {{ $mataPelajaran->jurusans->contains($j->id) ? 'checked' : '' }}>
